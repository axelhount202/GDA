...
class AdminController {
    ...
    public function assignCahierToProfesseur() {
        try {
            $cahiers = new Cahiers();
            $cahier = $cahiers->getCahierByID($_POST['cahier_id']);
    
            $professeurs = new Professeurs();
            $professeur = $professeurs->getProfesseurByID($_POST['professeur_id']);
    
            // Vérifier si l'affectation est possible
            ...
            if ($message['message'] === "Un cahier a été affecté à un professeur") {
                ...
                $affectations->createNewAssignment($_POST['cahier_id'], $_POST['professeur_id']);
            }

            // Redirection vers la page admin avec le nouveau message
