...
class Cahiers {
    ...
    public function deleteCahierByIdUtilisateur($ID_Utilisateur) {
        try {
            $stmt = $this->pdo->prepare("DELETE FROM Cahiers WHERE Id_Utilisateur = ?");
            $stmt->execute([$ID_Utilisateur]);

            return $stmt->rowCount();
        } catch (PDOException $e) {
            error_log("Erreur - deleteCahierByIdUtilisateur() : " . $e->getMessage());
            return false;
        }
    }
}
